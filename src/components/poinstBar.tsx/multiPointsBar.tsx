...
export const MultiPointsBar: FC<IMultiPointsBar> = ({ Points }) => {
    ...
    const maxPoints: number = useMemo(() => {
        return Points.reduce((a, b) => a + b.maxPoints, 0)
    }, [Points])
                ...
                return <Col style={{ width: `${percentage[i]}%`, backgroundColor: p.darkerColor }}>
                    {!!p.points || p.points === 0 ?
                        <Row style={{ position: "relative" }}>
                            <Col style={{ position: "absolute", backgroundColor: `${p.color}`, width: `${p.points / p.maxPoints * 100}%`,  }}></Col>
                            ...
                            <Col style={{ position: "absolute", backgroundColor: p.maxPoints > 0 ? p.color : p.darkerColor, width: `100%`,  }}></Col>
